...
def train(cfg, model, loss_cls, loss_func, device, train_loader, optimizer, epoch, logger, logstep, writer):
    model.train()
    ...
    for batch_idx, (data, labels) in enumerate(train_loader):
        ...
        loss_metric = loss_func(embeddings, labels)
...
def main(args):
    ...
    loss_func1 = losses.MultiSimilarityLoss(alpha=cfg['alpha'], beta=cfg['beta'], base=float(cfg['base']), distance=distance)
    mining_func1 = miners.MultiSimilarityMiner(epsilon=float(cfg['epsilon']))

    loss_func2 = losses.TripletMarginLoss(margin=0.2, distance=distance, reducer=reducers.ThresholdReducer(low=0))
    mining_func2 = miners.TripletMarginMiner(
        margin=float(cfg['margin']), distance=distance, type_of_triplets="semihard"
    )

    loss_func = losses.MultipleLosses([loss_func1, loss_func2], miners=[mining_func1, mining_func2])

    optimizer = optim.AdamW([
        {'params': model.parameters(), 'lr': float(cfg['lr'])},
        {'params': loss_cls.parameters(), 'lr': float(cfg['lr']) * 4},
    ...
    for epoch in tqdm(range(1, num_epochs + 1)):
        ...
        train(cfg, model, loss_cls, loss_func, device, train_loader, optimizer, epoch, logger, cfg['logstep'], writer)
