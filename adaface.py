...
class AdaFaceLoss(Module):
    def __init__(self,
        ...
        self.adaface = AdaFace(embedding_size=embedding_size,
                        classnum=classnum,
                        m=m,
                        h=h,
                        s=s,
    ...
    def forward(self, embbedings, label):
        ...
        return self.loss(self.adaface(normalized_embedding, norms, label), label)



if __name__ == '__main__':
    B = 16
    embbedings = torch.randn((B, 2048)).float()  # latent code
    labels =  torch.randint(6000, (B,))

    loss = AdaFaceLoss()

    out = loss(embbedings, labels)
    print(out)
