...
def train(cfg, model, loss_cls, loss_func, mining_func, loss_xbm_func, device, train_loader, optimizer, epoch, logger, logstep, writer):
    model.train()
    ...
    for batch_idx, (data, labels) in enumerate(train_loader):
        ...
        if epoch > 10: # warm up
            loss_metric = loss_xbm_func(embeddings, labels)
        else:
        ...
        if batch_idx % logstep == 0:
            if epoch > 10:
                logger.info(f'[Train XBM] Epoch {epoch} Iteration {batch_idx} : loss_metric = {loss_metric}, loss_classfiy = {loss_classfiy}')
            else:
                logger.info(f'[Train SM] Epoch {epoch} Iteration {batch_idx} : loss_metric = {loss_metric}, loss_classfiy = {loss_classfiy}')
...
def main(args):
    ...
    loss_xbm_func = losses.CrossBatchMemory(
        loss=loss_func,
        embedding_size=cfg['xbm_embedding_size'],
        memory_size=cfg['xbm_memory_size'],
        miner=mining_func
    )
    ...
    for epoch in tqdm(range(1, num_epochs + 1)):
        ...
        loss_xbm_func.reset_queue()
        train(cfg, model, loss_cls, loss_func, mining_func, loss_xbm_func, device, train_loader, optimizer, epoch, logger, cfg['logstep'], writer)
