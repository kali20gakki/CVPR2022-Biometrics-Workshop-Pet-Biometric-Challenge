...
def train(cfg, model, loss_cls, loss_func, mining_func, device, train_loader, optimizer, epoch, logger, logstep, writer):
    model.train()

    for batch_idx, (data, labels) in enumerate(train_loader):
        data, labels = data.to(device), labels.to(device)
        optimizer.zero_grad()
        embeddings, neck_out = model(data)

        indices_tuple = mining_func(embeddings, labels)
        loss_metric = loss_func(embeddings, labels, indices_tuple)

        loss_classfiy = loss_cls(neck_out, labels) * 0.01

        loss = loss_metric + loss_classfiy

        loss.backward()
        optimizer.step()

        if batch_idx % logstep == 0:
            logger.info(f'[Train] Epoch {epoch} Iteration {batch_idx} : loss_metric = {loss_metric}, loss_classfiy = {loss_classfiy}')
            writer.add_scalar('Loss', loss, global_step= (epoch - 1) * len(train_loader) + batch_idx)
            writer.add_scalar('loss_metric', loss_metric, global_step= (epoch - 1) * len(train_loader) + batch_idx)
            writer.add_scalar('loss_classfiy', loss_classfiy, global_step= (epoch - 1) * len(train_loader) + batch_idx)
...
def main(args):
    ...
    train_datset = BiometricsClsDataset2(data_source, cfg['train_images_dir'], 
                                        imgsz=cfg['imgsz'], p = 0.5, is_aug=True,
                                        use_w_cut=cfg['use_w_cut'])
    ...
    if cfg['model_name'] == 'MultiStageEmbeddingNetWithBNNeck':
        model = MultiStageEmbeddingNetWithBNNeck(stride=cfg['stride'], encoder_name=cfg['encoder_name']).to(device)
    elif cfg['model_name'] == 'MultiStageEmbeddingNetWithBNNeck':
        model = MultiStageEmbeddingNetWithBNNeckV2(stride=cfg['stride'], 
                                encoder_name=cfg['encoder_name'], 
                                embedding_size = cfg['embedding_size']).to(device)
    elif cfg['model_name'] == 'EmbeddingNetWithBNNeck':
        model = EmbeddingNetWithBNNeck(stride=cfg['stride'],
                                    encoder_name=cfg['encoder_name'], 
                                    embedding_size = cfg['embedding_size']).to(device)
    elif cfg['model_name'] == 'MultiStageEmbeddingNetV2':
        model = MultiStageEmbeddingNetV2(stride=cfg['stride'], 
                                encoder_name=cfg['encoder_name'], 
                                embedding_size = cfg['embedding_size']).to(device)
    elif cfg['model_name'] == 'EmbeddingNetLocalkWithBNNeck':
    ...
    if cfg['loss_cls_name'] == 'ArcFaceLoss':
        loss_cls = losses.ArcFaceLoss(num_classes = num_classes, embedding_size = cfg['embedding_size']).to(device)
    elif cfg['loss_cls_name'] == 'SubCenterArcFaceLoss':
        loss_cls = losses.SubCenterArcFaceLoss(num_classes = num_classes, 
                                            embedding_size = cfg['embedding_size'],
                                            margin=float(cfg['margin']), 
                                            scale=cfg['scale'], 
                                            sub_centers=cfg['sub_centers']).to(device)
    elif cfg['loss_cls_name'] == 'AdaFaceLoss':
        loss_cls = AdaFaceLoss(classnum = num_classes, 
                                embedding_size = cfg['embedding_size'],
                                m=float(cfg['m']),
                                h=float(cfg['h']),
                                s=float(cfg['s']),
                                t_alpha=float(cfg['t_alpha'])).to(device)
    elif cfg['loss_cls_name'] == 'NONE':
        print('no loss_cls')


    if cfg['loss_func_name'] == 'MultiSimilarityLoss':
    ...
    elif cfg['loss_func_name'] == 'CircleLoss':
        loss_func = losses.CircleLoss(m=0.4, gamma=80)

    optimizer = optim.AdamW([
        {'params': model.parameters(), 'lr': float(cfg['lr'])},
        {'params': loss_cls.parameters(), 'lr': float(cfg['lr']) * float(cfg['cls_lr_factor'])},
    ...
    if cfg['lr_scheduler'] == 'CosineAnnealingLR':
        ...
        lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, cfg['milestones'], gamma=0.1, last_epoch=-1)
        logger.info(f"use MultiStepLR ")
        warmup_scheduler = GradualWarmupScheduler(optimizer, multiplier=1, total_epoch=10, after_scheduler=lr_scheduler)
    ...
    for epoch in tqdm(range(1, num_epochs + 1)):
        ...
        train(cfg, model, loss_cls, loss_func, mining_func, device, train_loader, optimizer, epoch, logger, cfg['logstep'], writer)
