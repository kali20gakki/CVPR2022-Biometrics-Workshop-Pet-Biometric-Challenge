...
def train(cfg, model, loss_cls, device, train_loader, optimizer, epoch, logger, logstep, writer):
    model.train()
    ...
    for batch_idx, (data, labels) in enumerate(train_loader):
        ...
        global_feat, local_feat, neck_out = model(data)

        # indices_tuple = mining_func(embeddings, labels)
        # loss_metric = loss_func(embeddings, labels, indices_tuple)

        loss_metric = global_loss(TripletLoss(margin=0.3), global_feat, labels)[0] + \
...
def main(args):
    ...
    logger.info(f"cls_lr_factor = {cfg['cls_lr_factor']}")
    ...
    for epoch in tqdm(range(1, num_epochs + 1)):
        ...
        train(cfg, model, loss_cls, device, train_loader, optimizer, epoch, logger, cfg['logstep'], writer)
