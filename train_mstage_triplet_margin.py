...
def train(cfg, model, loss_func, mining_func, device, train_loader, optimizer, epoch, logger, logstep, writer):
    model.train()
    ...
    for batch_idx, (data, labels) in enumerate(train_loader):
        ...
        if batch_idx % logstep == 0:
            logger.info(f'[Train] Epoch {epoch} Iteration {batch_idx} : Loss = {loss}, mined triplets nums = {mining_func.num_triplets}')
            writer.add_scalar('Loss', loss, global_step= (epoch - 1) * len(train_loader) + batch_idx)
            writer.add_scalar('triplets nums', mining_func.num_triplets, global_step= (epoch - 1) * len(train_loader) + batch_idx)
...
def main(args):
    ...
    loss_func = losses.TripletMarginLoss(margin=float(cfg['margin']), distance=distance, reducer=reducer)
    ...
    for epoch in tqdm(range(1, num_epochs + 1)):
        ...
        train(cfg, model, loss_func, mining_func, device, train_loader, optimizer, epoch, logger, cfg['logstep'], writer)
